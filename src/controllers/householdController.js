...
exports.createHousehold = async (req, res) => {
  try {
    ...
    if (!familyName || !address) {
      return res
        .status(400)
        .json({ message: "Family name and address are required" });
    }
...
exports.submitSurvey = async (req, res) => {
  try {
    ...
    if (!household) {
      console.log(
        `Household with ID ${req.params.id} not found for survey submission`
      );
      return res.status(404).json({ message: "Household not found" });
    }
    ...
    const parsedPractices =
      typeof environmentalPractices === "string"
    ...
    console.error(
      `Error submitting survey for household ${req.params.id}:`,
      error
    );
    res.status(500).json({ message: "Server error", error: error.message });
  }
